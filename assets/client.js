...
window.journaler.sessionKey = () =>
  `${window.journaler.sessionKeyBase}_${window.journaler.feed()}`;

window.journaler.arrayBufferToHex = (buffer) =>
  [...new Uint8Array(buffer)]
    .map((byte) => byte.toString(16).padStart(2, "0"))
    .join("");

window.journaler.hexToArrayBuffer = (hex) => {
...
window.journaler.deriveKey = async () => {
  const keyMaterial = await window.journaler.getCryptoKey(
...
window.journaler.encryptText = async (text) => {
  const encoder = new TextEncoder();
  ...
  const encrypted = await crypto.subtle.encrypt(
    {
      name: "AES-GCM",
      iv: window.journaler.hexToArrayBuffer(window.journaler.iv),
    },
    key,
    encoder.encode(text),
  );
...
window.journaler.decryptText = async (encryptedData) => {
  const key = await window.journaler.deriveKey(
    window.journaler.password,
    window.journaler.hexToArrayBuffer(window.journaler.salt),
  );
...
window.journaler.loadPassword = () => {
  window.journaler.password = sessionStorage.getItem(
    window.journaler.sessionKey(),
  );
};
