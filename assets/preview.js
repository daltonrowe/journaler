...
const parseImage = (text) =>
  text.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (_, title, id) => {
    const imgEl = document.getElementById(id);
    const src = imgEl ? imgEl.src : "";
    const sizeMatch = id.match(/--(\d+)x(\d+)$/);
    const dims = sizeMatch
      ? ` width="${sizeMatch[1]}" height="${sizeMatch[2]}"`
      : "";
    ...
    switch (type) {
      case "list-detail": {
        ...
        if (lastIndex >= 0 && result[lastIndex].endsWith("</li>")) {
          result[lastIndex] =
            `${result[lastIndex].slice(0, -5)}<small>${parseInline(line.slice(3))}</small></li>`;
        }
        break;
      }
      case "list": {
        ...
        if (iconMatch) {
          result.push(
            `<li><span class="icon">${iconMatch[1]}</span> ${parseInline(iconMatch[2])}</li>`,
          );
        } else {
      ...
      case "h3": {
        const level = type[1];
        const text = line.slice(parseInt(level, 10) + 1);
        result.push(
          `<${type} id="${toId(text)}">${parseInline(text)}</${type}>`,
        );
        break;
      }
      case "image":
...
window.addEventListener("journaler-entry-ready", async () => {
  ...
  write.addEventListener("focus", debouncedRenderPreview);
  renderPreview();
});
